...
class Args(object):
    """ Option represents an Args flag in kubectl """
...
class CommandTree(object):
    ...
    def __init__(self, node=None, helptext=None, children=None, localFlags=None, localArgs=None):
        self.node = node
        self.help = helptext
        self.children = children if children else list()
        self.localFlags = localFlags if localFlags else list()
        self.localArgs = localArgs if localArgs else list()

    def __str__(self):
        return "Node: %s, Help: %s\n localFlags: %s\n localArgs: %s\n Children: %s" % (self.node, self.help, self.localFlags, self.localArgs ,self.children)
...
class Parser(object):
    ...
    def __init__(self, apiFile):
        ...
        self.ast = CommandTree("msgsend")
        self.ast = self.build(self.ast, self.schema.get("msgsend"))
    ...
    def build(self, root, schema):
        ...
        for name, desc in schema.get("options").items():
            if root.node == "msgsend":  # register global flags
                self.globalFlags.append(Option(name, desc["help"]))
            root.localFlags.append(Option(name, desc["help"]))
        for arg, desc in schema.get("args").items():
            root.localArgs.append(Args(arg, desc["help"]))
        return root
    ...
    def parse_tokens(self, tokens):
        ...
        """
        if len(tokens) == 1:
            return list(), tokens, {"msgsend": self.ast.help}
        else:
            tokens.reverse()
    ...
    def treewalk(self, root, parsed, unparsed):
        ...
        if root.node == token:
            ...
            if unparsed and not self.peekForOption(unparsed) and self.peekForArg(root, unparsed): # check for Args
                logger.debug("arg(s) upcoming %s", unparsed)
                parsed_args, unparsed, suggestions = self.evalArgs(root, list(), unparsed[:])
                if(parsed_args):
                    logger.debug("parsed arg(s): %s", parsed_args)
                    parsed.extend(parsed_args)
            if unparsed and not self.peekForOption(unparsed) and not self.peekForArg(root, unparsed):  # unparsed bits without options
                ...
                for child in root.children:
                    ...
                    # return args
                    for arg in root.localArgs:
                        suggestions[arg.name] = arg.helptext
        else:
    ...
    def peekForArg(self, root, unparsed):
        """ Peek to find out if next token is an option """
        if unparsed:
            token = unparsed[-1]
            for arg in root.localArgs:
    ...
    def evalArgs(self, root, parsed, unparsed):
        """ Evaluate only the Args and return flags as suggestions """
        logger.debug("parsing Args at tree: %s with p:%s, u:%s", root.node, parsed, unparsed)
        suggestions = dict()
        token = unparsed.pop().strip()
        ...
        allFlags = root.localArgs
        for flag in allFlags:
            if flag.name == token:
                logger.debug("matched token: %s with flag: %s", token, flag.name)
                parsed.append(token)
                if self.peekForArg(root, unparsed):  # recursively look for further options
                    parsed, unparsed, suggestions = self.evalArgs(root, parsed, unparsed[:])
                # elif token == "--namespace":
                #     namespaces = [('default', None), ('minikube', None), ('gitlab', None)]  # self.kube_client.get_resource("namespace")
                #     suggestions = dict(namespaces)
                break
        ...
        if suggestions:  # incomplete parse, replace token
            logger.debug("incomplete Args: %s provided. returning suggestions", token)
            unparsed.append(token)
        return parsed, unparsed, suggestions
